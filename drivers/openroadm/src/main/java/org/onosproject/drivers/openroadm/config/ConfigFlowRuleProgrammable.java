 ...
 */
package org.onosproject.drivers.openroadm.config;
...
public class ConfigFlowRuleProgrammable
    ...
    public Collection<FlowRule> applyFlowRules(Collection<FlowRule> rules) {
        ...
        config.map(cfg -> cfg.flowtable(table))
              .ifPresent(FlowTableConfig::apply);
    ...
    public boolean containsFlowRules(Collection<FlowRule> rules) {
        log.trace("containsFlowRules: {}", rules);
        Optional<FlowTableConfig> config = getFlowTableConfig();
        Set<FlowRule> table = new LinkedHashSet<>(getFlowTable(config));
        return table.containsAll(rules);
    }
